...
## set lib and python path for SONiC apps
sudo LANG=C chroot $FILESYSTEM_ROOT /bin/bash -c 'echo "export LD_LIBRARY_PATH=/usr/lib/x86_64-linux-gnu/" >> /etc/profile'
sudo LANG=C chroot $FILESYSTEM_ROOT /bin/bash -c 'echo "export PYTHONPATH=/usr/lib/sonic:/usr/lib/x86_64-linux-gnu/sonic" >> /etc/profile'
